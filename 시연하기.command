...
# 첫 화면 / 국회의원 컬렉션(collection/) / 국회기록물(records/) › 구술기록(records/oral-history/) / 기록콘텐츠(contents/)
# (JSON 데이터·자막·Google 지도는 파일을 바로 열면 동작하지 않으므로 서버로 엶)
# Google 지도 키는 http://localhost:8000/* 로 제한되어 있으므로 8000번 포트를 씀
cd "$(dirname "$0")"
PORT=8000
URL="http://localhost:$PORT/"
if curl -s "${URL}records/oral-history/config.js" 2>/dev/null | grep -q "OH_CONFIG"; then
...
echo "첫 화면: $URL"
echo "구술기록: ${URL}records/oral-history/"
( sleep 1; open "$URL" ) &
